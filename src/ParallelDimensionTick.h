...
struct DimensionWorkerContext {
    ...
    // 异步 tick 支持
    std::atomic<bool>           isTickingNow{false};
    std::atomic<uint64_t>       tickNumber{0};  // 当前正在处理的 tick 编号
    std::atomic<uint64_t>       completedTickNumber{0};  // 已完成的 tick 编号
    std::atomic<uint64_t>       skippedTicks{0};  // 跳过的 tick 数量
};
...
public:
    ...
    struct Stats {
        ...
        std::atomic<uint64_t> totalSkippedTicks{0};  // 总跳过的 tick 数
    };
...
private:
    ...
    std::atomic<uint64_t>                           mGlobalTickNumber{0};
